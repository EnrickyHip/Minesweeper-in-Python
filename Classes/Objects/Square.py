...
class Square(pygame.sprite.Sprite):
    ...
    def get_neighbors_bombs(self):
        self.neighbors = Matrix.get_neighbors(
            self.x, self.y, self.table.squares)
        filtered_neighbors = filter(
            lambda neighbor: neighbor.is_bomb, self.neighbors)
    ...
    def update(self):
        ...
        if self.is_opened:
            self.image = pygame.image.load(
                Square.number_images[self.neighbors_bombs])

        elif not self.table.alive:
            if self.exploded:
                self.image = pygame.image.load(
                    'images/square-bomb-explode.png')

            elif self.is_flagged:
                if not self.is_bomb:
                    self.image = pygame.image.load(
                        'images/square-wrong-flag.png')
